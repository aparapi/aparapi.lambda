...
byte_t *CodeAttribute::getCode(){
   return(code);
}
u4_t CodeAttribute::getCodeLength(){
   return(code_length);
}
...
char *ClassInfo::getClassName(){
      ...
      UTF8ConstantPoolEntry *thisClassUTF8ConstantPoolEntry = (UTF8ConstantPoolEntry*)constantPool[thisClassConstantPoolEntry->getNameIndex()];
      //fprintf(stdout, "UTF8 at this class name index is \"%s\"\n", thisClassUTF8ConstantPoolEntry->getUTF8Bytes());
   return((char*)thisClassUTF8ConstantPoolEntry->getUTF8Bytes());
}
...
char *ClassInfo::getSuperClassName(){
   ...
   return((char*)superClassUTF8ConstantPoolEntry->getUTF8Bytes());
}
