...
public class OkraKernel {
	...
	private boolean pushPrimitiveArg(Class<?>argclass, Object arg) {
		if (argclass.equals(Float.class)) pushFloatArg((float) arg);
		else if (argclass.equals(Integer.class)) pushIntArg((int) arg);
		else if (argclass.equals(Long.class)) pushLongArg((long) arg);
		else if (argclass.equals(Double.class)) pushDoubleArg((double) arg);
		else if (argclass.equals(Boolean.class)) pushBooleanArg((boolean) arg);
		else if (argclass.equals(Byte.class)) pushByteArg((byte) arg);
		else return false;
	...
	public int dispatchWithArgsUsingRawArrays(Object ... args) {
		...
		for (Object arg : args) {
			...
			// System.out.println("argclass=" + argclass);
			if (!pushPrimitiveArg(argclass, arg)) {
				if (argclass.equals(float[].class)) pushFloatArrayArg((float[]) arg);
				else if (argclass.equals(int[].class)) pushIntArrayArg((int[]) arg);
				else if (argclass.equals(long[].class)) pushLongArrayArg((long[]) arg);
				else if (argclass.equals(double[].class)) pushDoubleArrayArg((double[]) arg);
				else if (argclass.equals(boolean[].class)) pushBooleanArrayArg((boolean[]) arg);
				else if (argclass.equals(byte[].class)) pushByteArrayArg((byte[]) arg);
